...
module "jumphost" {
  ...
  user_data_extra = <<-EOT
    apt-get update -y
    apt-get install -y mtr netcat dnsutils curl jq
  EOT

  tags = local.tags
}

# Backend service using workload module
module "backend" {
  source = "../../modules/workload"
  ...
  replicas         = 2
  create_service   = true
  create_ingress   = true       # enabled ingress to allow curl from jumphost
  ingress_scheme   = "internal" # internal ALB for backend service

  containers = [{
    name  = "backend"
    image = "hashicorp/http-echo:0.2.3"
    args  = ["-text=Hello from Backend Service Discovery"]
  }]
...
# Frontend service that discovers backend
module "frontend" {
  source = "../../modules/workload"
  ...
  containers = [{
    ...
    args = [
      ...
      while true; do
        ...
        nslookup backend.service-discovery.svc.cluster.local || echo "DNS resolution failed"

        echo "Testing HTTP call to backend..."
        curl -s http://backend.service-discovery.svc.cluster.local:80 || echo "HTTP call failed"

        echo "Testing short name resolution..."
        curl -s http://backend:80 || echo "Short name resolution failed"
...
output "service_discovery_commands" {
  description = "Commands to test service discovery"
  value = {
    "frontend_connect"      = "kubectl exec -n service-discovery deployment/frontend -it -- /bin/sh"
    "frontend_curl"         = "kubectl exec -n service-discovery deployment/frontend -- curl -s http://backend:80"
    "jumphost_connect"      = "aws ssm start-session --region ${data.aws_region.current.region} --target ${module.jumphost.instance_id != null ? module.jumphost.instance_id : null}"
    "backend_from_jumphost" = module.backend.alb_url != null ? "curl -s http://${module.backend.alb_dns_name}" : null
    "check_logs_frontend"   = "kubectl logs -n service-discovery deployment/frontend"
    "check_logs_backend"    = "kubectl logs -n service-discovery deployment/backend"
  }
}
